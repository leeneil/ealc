...
def evaluate_train_data(sess,x,y,y_,keep_prob,train_data_files, bath_size, correct_prediction, num_classes, image_batch, label_batch):     
    ...
    for s in range(num_batches):
        ...
        nature_labels =   tf.argmax(train_labels, axis=1)
        nature_predicts = tf.argmax(train_predictions, axis=1)
...
def main(tfrecords_dir, model_output_dir, num_train_steps, bath_size, print_steps, evaluate_steps, image_size):
    ...
    # First convolutional layer. 16 feature maps.
    ...
    # Second convolutional layer. 32 feature maps.
    ...
    # Fully connected layer. Here we choose to have 512 neurons in this layer.
    h_pool_flat = tf.reshape(h_pool3, [-1, int(image_size/8*image_size/8*64)])
    W_fc1 = weight_variable([int(image_size/8*image_size/8*64), 512])
    ...
    with tf.Session() as sess: 
        ...
        for step in range(num_train_steps):
            ...
            if step % print_steps == 0:
                ...
                print( "{:%I:%M %p} ".format(datetime.datetime.today()) + "Step %d, Training Accuracy %g"%(step, float(train_accuracy)) )
